...
- Raspberry Pi web UI discovery (`2026-04-23`):
  ...
  - Strategy APR tracking (`2026-04-24`): the Pi dashboard now persists per-bot strategy performance to `/var/lib/lp-ranger/bots/<id>/.local/share/lp-ranger/strategy_performance.json`. The tracker closes and reopens sessions on bot start/stop and on strategy changes, and accumulates real deltas of `fees` and `IL` while that strategy was active.
  - APR accounting caveat (`2026-04-24`): the current web tracker excludes external wallet transfers by design because it does not use wallet balance changes as profit. It also does not count pool top-ups as direct PnL. However, it is still not a strict time-weighted-return engine for pool cash flows; APR is annualised from strategy `PnL = fees - IL` over observed pool capital, so future work should record explicit pool cash-in/cash-out events if accounting-grade strategy comparisons are required.
